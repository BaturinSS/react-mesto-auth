    ...
    <div className={`authorization authorization_type_${name}`}>
      ...
      >
        <input
          ...
          name={`${name}Email`}
        />
        <span className={`${name}Email-input-error authorization__input-error`}></span>
        <input
          ...
          maxLength="12"
        />
        <span className={`${name}Password-input-error authorization__input-error`}></span>
        <button
      ...
      <p className="authorization__comment">Уже зарегистрированы?<a className="authorization__link-entry"> Войти</a></p>
    </div>
  )
}
